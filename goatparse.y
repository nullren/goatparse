...
Decrement: '-' Duration { $$ = -$2.(time.Duration) }

Duration: INTEGER Duration1 { $$ = time.Duration($1.(int)) * $2.(time.Duration) }
...
Duration1:
  ...
  | DAY { $$ = time.Hour * 24 }
  | WEEK { $$ = time.Hour * 24 * 7 }
  | FORTNIGHT { $$ = time.Hour * 24 * 7 * 2 }

Increment: '+' Duration { $$ = $2.(time.Duration) }

Goatparse: Duration Goatparse1 { $$ = $1.(time.Duration) + $2.(time.Duration) }

Goatparse1:
  /* EMPTY */ { $$ = time.Duration(0) }
  | Goatparse1 Goatparse11 { $$ = $1.(time.Duration) + $2.(time.Duration) }
...
func (l *GoatparseLex) Lex(lval *GoatparseSymType) int {
  ...
  if tt == "d" || tt == "day" || tt == "days" { return DAY }
  if tt == "h" || tt == "hour" || tt == "hours" { return HOUR }
  if tt == "m" || tt == "min" || tt == "mins" || tt == "minute" || tt == "minutes" { return MINUTE }
  if tt == "second" || tt == "s" || tt == "seconds" { return SECOND }
  if tt == "week" || tt == "weeks" || tt == "w" { return WEEK }
  if tt == "fortnight" || tt == "fortnights" { return FORTNIGHT }
  return int(tok)
}
